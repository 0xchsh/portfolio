            ...
            {projects.map((item, i) => (
              <div key={item.name} className="flex items-center gap-2 min-w-0">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  {item.status === 'live' && (
                    <span className="flex items-center gap-1 text-sm text-neutral-400">Live<span className="relative flex h-1.5 w-1.5" style={{ top: 1 }}><span className="absolute -inset-0.5 rounded-full bg-green-400" style={{ animation: 'slow-ping 4s ease-out infinite', animationDelay: `${[0, 0, 0, 1000, 2000, 3000, 0, 0, 0][i]}ms`, animationFillMode: 'backwards' }} /><span className="relative rounded-full h-1.5 w-1.5 bg-green-500" /></span></span>
                  )}
